...
export async function GET(request: NextRequest) {
  ...
  if (favoritesOnly) {
    ...
    if (q && mine) {
      ...
      rawSql = isAdmin
        ? sql`SELECT s.id, s.title, s.artist, s.cover_url, s.spotify_track_id, s.spotify_album, s.created_by, s.created_by_name, s.is_public, s.public_requested, s.created_at, s.updated_at, s.lyrics_needs_review FROM songs s INNER JOIN favorites f ON f.song_id = s.id AND f.user_email = ${userEmail} ORDER BY s.updated_at DESC`
  ...
  if (q && mine && user) {
    const pattern = `%${q}%`;
    const songs = isAdmin
      ? await db.all(sql`SELECT id, title, artist, cover_url, spotify_track_id, spotify_album, created_by, created_by_name, is_public, public_requested, created_at, updated_at, lyrics_needs_review FROM songs WHERE (title LIKE ${pattern} OR artist LIKE ${pattern}) AND created_by = ${userEmail} ORDER BY updated_at DESC`) as unknown as SongListItem[]
      : await db.all(sql`SELECT id, title, artist, cover_url, spotify_track_id, spotify_album, created_by, created_by_name, is_public, public_requested, created_at, updated_at, lyrics_needs_review FROM songs WHERE (title LIKE ${pattern} OR artist LIKE ${pattern}) AND created_by = ${userEmail} AND (is_public = 1 OR created_by = ${userEmail}) ORDER BY updated_at DESC`) as unknown as SongListItem[];
    return NextResponse.json(songs);
  } else if (q) {
    const pattern = `%${q}%`;
    const songs = isAdmin
      ? await db.all(sql`SELECT id, title, artist, cover_url, spotify_track_id, spotify_album, created_by, created_by_name, is_public, public_requested, created_at, updated_at, lyrics_needs_review FROM songs WHERE (title LIKE ${pattern} OR artist LIKE ${pattern}) ORDER BY updated_at DESC`) as unknown as SongListItem[]
      : await db.all(sql`SELECT id, title, artist, cover_url, spotify_track_id, spotify_album, created_by, created_by_name, is_public, public_requested, created_at, updated_at, lyrics_needs_review FROM songs WHERE (title LIKE ${pattern} OR artist LIKE ${pattern}) AND is_public = 1 ORDER BY updated_at DESC`) as unknown as SongListItem[];
    return NextResponse.json(songs);
  } else if (mine && user) {
    const songs = isAdmin
      ? await db.all(sql`SELECT id, title, artist, cover_url, spotify_track_id, spotify_album, created_by, created_by_name, is_public, public_requested, created_at, updated_at, lyrics_needs_review FROM songs WHERE created_by = ${userEmail} ORDER BY updated_at DESC`) as unknown as SongListItem[]
      : await db.all(sql`SELECT id, title, artist, cover_url, spotify_track_id, spotify_album, created_by, created_by_name, is_public, public_requested, created_at, updated_at, lyrics_needs_review FROM songs WHERE created_by = ${userEmail} AND (is_public = 1 OR created_by = ${userEmail}) ORDER BY updated_at DESC`) as unknown as SongListItem[];
    return NextResponse.json(songs);
  } else {
    const songs = isAdmin
      ? await db.all(sql`SELECT id, title, artist, cover_url, spotify_track_id, spotify_album, created_by, created_by_name, is_public, public_requested, created_at, updated_at, lyrics_needs_review FROM songs ORDER BY updated_at DESC`) as unknown as SongListItem[]
      : await db.all(sql`SELECT id, title, artist, cover_url, spotify_track_id, spotify_album, created_by, created_by_name, is_public, public_requested, created_at, updated_at, lyrics_needs_review FROM songs WHERE is_public = 1 ORDER BY updated_at DESC`) as unknown as SongListItem[];
    return NextResponse.json(songs);
  }
}
